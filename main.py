...
class Dialog():
    def __init__(self, user_id):
        ...
        self.response['response']['text'] = 'Привет! \n Добро пожаловать в навык "Вечерние Прогулки". \n Назвав свой город и улицу, на которую вы хотите пойти, вы получите информацию о том, подходящая ли погода для прогулки, и карту нужной вам улицы, чтобы вы смогли продумать маршрут.\nСкажите название своего города, чтобы начать прогулку!'
    ...
    def make_dialog_line(self, req):
        ...
        if self.stage == 1:
            if req['request']['original_utterance'].lower() in ['что ты умеешь?', 'помощь']:
                self.response['response']['text'] = '1. Скажите название своего города, если окажется, что погода не подходит для вечерней прогулки, я вам сообщу. \n2. Укажите как вы будете передвигаться: на транспортном средстве или пешком. В зависимости от этого я дам вам информацию о пробках в этом районе\n 3.  Назовите улицу, на которую собираетесь пойти. Я отправлю вам карту указанной улицы. \n4. Я спрошу у вас хотите ли вы найти что-нибудь'
                self.response['response']['tts'] = 'Скажите название своего города, если окажется, что погода не подходит для вечерней прогулки, я вам сообщу. \n Ук+ажите как вы б+удете передвиг+аться: на транспортном средстве или пешком. В зависимости от этого я дам вам информ+ацию о пр+обках в этом рай+оне\n П+осле этого назов+ите улицу, на кот+орую собир+аетесь пойти. Я отпр+авлю вам к+арту ук+азанной улицы.'
                return
